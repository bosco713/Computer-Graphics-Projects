...
void drawRightAngledTrianglePrism(double length, double height, double thickness) {
    ModelerDrawState *mds = ModelerDrawState::Instance();
    ...
    if (mds->m_rayFile)
    {
        _dump_current_modelview();
        fprintf(mds->m_rayFile, 
            "RightAngledTrianglePrism { length=%f, height=%f, thickness=%f;\n", length, height, thickness );
        _dump_current_material();
        fprintf(mds->m_rayFile, "})\n" );
    }
    else
    {
        glPushMatrix();
            // drawTriangle(0, 0, 0, length, 0, 0, 0, height, 0);          // draw the base triangle
            glPushMatrix();
                glRotated(180, 0, 1, 0);
                drawTriangle(0, 0, 0, 0, height, 0, -1 * length, 0, 0);     // flap the triangle inside out to avoid shading bugs caused by the library

            glPopMatrix();
            drawTriangle(0, 0, thickness, length, 0, thickness, 0, height, thickness);  // draw the top triangle
            drawBox(length, 0.1f, thickness);                           // draw the length face
            drawBox(0.1f, height, thickness);                           // draw the height face
            glPushMatrix();
                glTranslated(length, 0, 0);
                glRotated(180-atan(height/length)/M_PI * 180, 0, 0, 1);
                drawBox(sqrt(pow(length,2)+pow(height,2)), 0.1f, thickness);        // draw the hypothesis face
...
void drawTorus(double radius, double thickness) {
    ModelerDrawState* mds = ModelerDrawState::Instance();
    ...
    else
    {
        ...
        for (int i = 0; i < 360; ++i) {
            glPushMatrix();
                glTranslated(radius * cos((double)i / 180 * M_PI), 0, radius * sin((double)i / 180 * M_PI));
                drawSphere(thickness / 2);
...
void drawStar(double size, double flip_in_rotation, double angle_of_triangle) {
    ModelerDrawState* mds = ModelerDrawState::Instance();
    ...
    if (mds->m_rayFile)
    {
        _dump_current_modelview();
        fprintf(mds->m_rayFile,
            "Star { size=%f, flip_in_rotation=%f, angle_of_triangle=%f;\n", size, flip_in_rotation, angle_of_triangle);
        _dump_current_material();
        fprintf(mds->m_rayFile, "})\n");
    }
    else
    {
        glPushMatrix();
            ...
            glPushMatrix();
                // glTranslated(0, 4, 0);
                glRotated(180, 0, 0, 1);
                glTranslated(0, -size + sin((double)flip_in_rotation / 180 * M_PI) * cos((double)angle_of_triangle / 180 * M_PI) - 0.04f, 0);
                glPushMatrix();
                    glTranslated(0, size, 0);
                    glRotated(atan(sin((double)flip_in_rotation / 180 * M_PI) * cos((double)angle_of_triangle / 180 * M_PI) / sin((double)angle_of_triangle / 180 * M_PI)) * 180 / M_PI, 1, 0, 0);
                    glPushMatrix();
