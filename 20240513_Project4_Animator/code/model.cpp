...
public:
	...
	void drawParticles(Mat4f cameraMatrix, int numParticles)
	{
		Mat4f worldMatrix = cameraMatrix.inverse() * getModelViewMatrix();
		Vec4f pos = worldMatrix * Vec4f(0, 0, 0, 1);
		ParticleSystem* ps = ModelerApplication::Instance()->GetParticleSystem();
		ps->spawnParticles(Vec3f(pos[0], pos[1], pos[2]), numParticles);
	}
};
...
void ourModel::draw()
{
	...
	double pi = 360 * (ModelerApplication::Instance()->GetFps() / 49.0f) * M_PI / 180;
...
int main()
{
	...
	controls[PARTICLE_NUM] = ModelerControl("Number of particles", 1, 50, 1, 1);
	/*
