...
const float kMouseTwistSensitivity			= 0.05f;
...
void Camera::calculateViewingTransformParameters() 
{
	...
	float twistValue = getTwist();
	float cosTwist = cos(twistValue);
	float sinTwist = sin(twistValue);

	if ( fmod((double)mElevation, 2.0*M_PI) < 3*M_PI/2 && fmod((double)mElevation, 2.0*M_PI) > M_PI/2 )
		mUpVector= Vec3f(sinTwist,-cosTwist,0);
	else
		mUpVector= Vec3f(sinTwist,cosTwist,0);
...
void Camera::lookAt(Vec3f eye, Vec3f at, Vec3f up) {
	Vec3f F_norm_vector = at - eye;
	F_norm_vector.normalize();
	Vec3f UP_norm_vector = up;
	UP_norm_vector.normalize();
	Vec3f s = F_norm_vector ^ UP_norm_vector;
	Vec3f s_norm_vector = s;
	s_norm_vector.normalize();
	Vec3f u = s_norm_vector ^ F_norm_vector;
	F_norm_vector = F_norm_vector * -1;			// *-1 once first to fit in the matrix M
	GLfloat M[16] = {s[0],	u[0],	F_norm_vector[0],	0,
					 s[1],	u[1],	F_norm_vector[1],	0,
					 s[2],	u[2],	F_norm_vector[2],	0,
					 0,		0,		0,					1 };		// somehow need a transpose, idk why
	glMultMatrixf(M);
