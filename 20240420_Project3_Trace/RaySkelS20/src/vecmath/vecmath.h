...
inline vec3f sampleHemisphere(const vec3f& R, double shininess) {
	double u1 = getRandomReal();
	double u2 = getRandomReal();

	double power = pow(u1, 1.0 / (shininess + 1));
	double phi = 2 * PI * u2;
	double x = cos(phi) * sqrt(1 - power * power);
	double y = sin(phi) * sqrt(1 - power * power);
	double z = power;

	vec3f Z = R.normalize();
	vec3f H = abs(R[0]) > 0.9 ? vec3f(0, 1, 0) : vec3f(1, 0, 0);
	vec3f X = H.cross(Z).normalize();
	vec3f Y = Z.cross(X);
...
inline vec3f cosHemiRandom(const vec3f& n) {
	float p = 2 * PI * getRandomReal();

	float cos_t = sqrt(getRandomReal());
	...
	if (fabs(n[0]) > fabs(n[1]))
		basis1 = vec3f(n[2], 0, -n[0]).normalize();
	else
		basis1 = vec3f(0, -n[2], n[1]).normalize();

	basis2 = n.cross(basis1);

	return randomVec[0] * basis1 + randomVec[1] * basis2 + randomVec[2] * n;
}

inline vec3f lerp(vec3f a, vec3f b, double f)
{
	return a + f * (b - a);
}
