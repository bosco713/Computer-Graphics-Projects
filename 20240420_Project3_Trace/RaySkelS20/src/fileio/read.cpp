...
static void processObject( Obj *obj, Scene *scene, mmap& materials )
{
	...
	if( name == "directional_light" ) {
		...
		double const_att = 1.0;
		double linear_att = 1.0/20;
		double quad_att = 1.0/400;
		if (hasField(child, "constant_attenuation_coeff")) {
			const_att = getField(child, "constant_attenuation_coeff")->getScalar();
		}
		if (hasField(child, "linear_attenuation_coeff")) {
			linear_att = getField(child, "linear_attenuation_coeff")->getScalar();
		}
		if (hasField(child, "quadratic_attenuation_coeff")) {
			quad_att = getField(child, "quadratic_attenuation_coeff")->getScalar();
		}
	...
	else if (name == "ambient_light")
	{
		...
		scene->addAmbientLightColor(tupleToVec(getColorField(child)));
	} 
		...
		double linear_att = 1.0 / 20;
		double quad_att = 1.0 / 400;
		vec3f edgeplace = vec3f(0.0, 0.0, 0.0);	// later reset
		...
		if (hasField(child, "edgeplace")) {
			edgeplace = tupleToVec(getField(child, "edgeplace"));
			cutOffAngle = edgeplace[0];
		}
