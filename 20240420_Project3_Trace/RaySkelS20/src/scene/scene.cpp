...
int Scene::getDepth() {
	return traceUI->getDepth();
}

double Scene::getConstantAttenuation() {
	return traceUI->getConstantAttenuation();
}

double Scene::getLinearAttenuation() {
	return traceUI->getLinearAttenuation();
}

double Scene::getQuadraticAttenuation() {
	return traceUI->getQuadraticAttenuation();
}

double Scene::getAmbientLight() {
	return traceUI->getAmbientLight();
}

bool Scene::getGlossy() {
	return traceUI->getGlossy();
}
vec3f Scene::getAmbientLightColor() {
	return ambientLightColor;
}

void Scene::addAmbientLightColor(const vec3f& amb) {
	ambientLightColor += amb;
	ambientLightColor.clamp();
}

bool Scene::lineOfSight(const vec3f& from, const vec3f& to) const {
	vec3f direction = to - from;
	float distance = direction.length();
	direction.normalize();

	ray sightRay(from, direction);
	isect i;

	for (auto obj : objects) {
		if (obj->intersect(sightRay, i)) {
			if (i.t < distance) {
				return false;
			}
		}
