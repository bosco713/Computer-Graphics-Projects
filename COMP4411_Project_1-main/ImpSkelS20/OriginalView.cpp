...
void OriginalView::draw()
{
	...
	if ( m_pDoc->m_ucBitmap ) 
	{
		...
		// After drawing the image, draw the marker if coordinates are within bounds
		if (markerPos.x >= 0 && markerPos.y >= 0 && markerPos.x < m_nWindowWidth && markerPos.y < m_nWindowHeight) {
			// Set marker color and size
			glColor3f(1.0, 0.0, 0.0); // Red color for visibility
			glPointSize(5.0); // Size of the point marker

			// Draw the marker
			glBegin(GL_POINTS);
				glVertex2i(markerPos.x, m_nWindowHeight - markerPos.y);
			glEnd();
		}
...
void OriginalView::setMarkerPos(int x, int y) {
	// Update marker position
	markerPos.x = x;
	markerPos.y = y;

	// Refresh the view to show the marker
	refresh();
}
